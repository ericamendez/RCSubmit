...
const DueThisWeek = ({ currentWeek }) => {
  ...
  const [deleteAssignment] = useMutation(DELETE_ASSIGNMENT, {
    onCompleted: (response) => {
      ...
      setCurrentWeekAssignments(
        currentWeekAssignments.filter((a) => a.id !== deletedAssignmentId),
      );
    },
    onError: (error) => {
  ...
  const handleEditAssignmentSubmit = async (e, id, show) => {
    e.preventDefault();
    await editAssignment({
      variables: { id, description, link, show, assignmentType },
    });
  };
              ...
              <li key={assignment.id}>
                <div>
                  <FontAwesomeIcon
                    icon={faCircleCheck}
                    className={
                      assignment && assignment.show ? "chaeck green" : "check"
                    }
                    onClick={(e) =>
                      handleShowAssignment(e, assignment.id, assignment.show)
                    }
                  />
                  <p>
                ...
                <div className="editAssignmentIcons">
                  <FontAwesomeIcon
                    icon={faPenToSquare}
                    className={
                      assignment.id === isEditID
                        ? "penToSquare editSelected"
                        : "penToSquare"
                    }
                    onClick={(e) =>
                  ...
                  <FontAwesomeIcon
                    ...
                    className="trash"
                    onClick={() => {
                      deleteAssignment({ variables: { id: assignment.id } });
                    }}
                  />
                </div>
                  ...
                  >
                    <ul>
                      ...
                      <li>
                        ...
                        <select
                          onChange={({ target }) =>
                            setAssignmentType(target.value)
                          }
                        >
                          {assignmentTypes.map((type, i) =>
                            type === assignment.assignmentType ? (
                              <option key={i} value={type} selected>
                                {type}
                              </option>
                            ) : (
        ...
        <form onSubmit={handleSubmitAssignment}>
          <ul>
            ...
            <li>
              Assignment Type:
              <select
                onChange={({ target }) => setAssignmentType(target.value)}
              >
                {assignmentTypes.map((type, i) => (
                  <option key={i} value={type}>
                    {type}
